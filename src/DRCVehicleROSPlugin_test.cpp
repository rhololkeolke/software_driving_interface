...
void DRCVehicleROSPlugin::SetDrivingState(const driving_msgs::HDI_control::ConstPtr
   &_msg)
{
   // wheel angle
   ROS_INFO("Output generated by SetDrivingState function.");
//static_cast< void (DRCVehicleROSPlugin::*) (const std_msgs::Float64::ConstPtr&) >
//   std_msgs::Float64 wheelAngle = static_cast<std_msgs::Float64>(_msg->wheel_angle);
//   DRCVehicleROSPlugin::SetHandWheelState(static_cast< void (DRCVehicleROSPlugin::*) (const std_msgs::Float64::ConstPtr&) > (wheelAngle));

   // gas position
//   DRCVehicleROSPlugin::SetGasPedalPercent(_msg->gas_pos);

   // brake position
//   DRCVehicleROSPlugin::SetBrakePedalPercent(_msg->brake_pos);

   // gear
//   DRCVehicleROSPlugin::SetDirectionState(_msg->gear);
...
///////////////////////////////////////////////////////////////////////
   ROS_INFO("ROS_INFO before pub and subscriber are initialized.");
   printf("Printf before pub and subscriber are initialized.\n");

	// method: SetDrivingState
	// var: subHDICmd
   ros::SubscribeOptions HDI_cmd_so =
      ros::SubscribeOptions::create< driving_msgs::HDI_control >(
      this->model->GetName() + "/HDI/cmd", 100,
      boost::bind(static_cast< void (DRCVehicleROSPlugin::*)
         (const driving_msgs::HDI_control::ConstPtr&) >(
            &DRCVehicleROSPlugin::SetDrivingState), this, _1),
      ros::VoidPtr(), &this->queue);
   this->subHDICmd = this->rosNode->subscribe(HDI_cmd_so);

	// var: pubHDIState
   this->pubHDIState = this->rosNode->advertise< driving_msgs::HDI_feedback >(
      this->model->GetName() + "/HDI/state", 10);

   ROS_INFO("ROS_INFO after pub and subscriber are initialized.");
   printf("Printf after pub and subscriber are initialized.\n");
